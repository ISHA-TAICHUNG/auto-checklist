    ...
    switch (api) {
      ...
      case "admin": {
        ...
        const DESTRUCTIVE_ACTIONS = [
          "cleanupAll",
          "cleanupDate",
          "cleanupOfficialDocumentMonitor",
        ];
        if (
          WRITE_ACTIONS.indexOf(action) >= 0 ||
          DESTRUCTIVE_ACTIONS.indexOf(action) >= 0
        ) {
        ...
        if (
          DESTRUCTIVE_ACTIONS.indexOf(action) >= 0 &&
          destructiveActionWillDelete_(action, e.parameter) &&
          !destructiveHttpAllowed_()
        ) {
          throw new Error(
            "未授權：破壞性 action 預設禁止 HTTP 呼叫，請在 Apps Script 編輯器手動執行函式（或設 Script Property ALLOW_DESTRUCTIVE_HTTP=YES）",
          );
        }
        switch (action) {
          case "formatSheets": {
          ...
          case "runInit": {
            ...
            result = {
              ok: true,
              action,
              message: "initializeDatabase 執行完成",
            };
            break;
          }
          case "applyDropdowns": {
          ...
          case "migrateFixedCraneMonthlyChecklist": {
            result = {
              ok: true,
              action,
          ...
          case "lineTargetStatus": {
            const cfg =
              typeof getLineConfig_ === "function" ? getLineConfig_() : null;
            const subscriberIds =
              typeof getLineSubscriberUserIds_ === "function"
                ? getLineSubscriberUserIds_()
                : [];
            const legacyTargetUserCount = cfg
              ? Array.from(new Set(cfg.userIds || [])).length
              : 0;
            const supervisorIds =
              typeof getSupervisorUserIds_ === "function"
                ? getSupervisorUserIds_()
                : [];
            const supervisorTargetCount = Array.from(
              new Set(supervisorIds || []),
            ).length;
            result = {
              ok: true,
              action,
              ...
              subscriberTargetCount: Array.from(new Set(subscriberIds || []))
                .length,
          ...
          case "testLineIncident": {
            ...
            try {
              ...
              if (
                cfg &&
                cfg.token &&
                typeof sendIncidentAlert_ === "function"
              ) {
          ...
          case "cleanupAll": {
            ...
            const summary = cleanupAllSubmissionsAndIncidents_({
              dryRun,
              confirm,
            });
          ...
          case "systemStatus": {
            ...
            result = {
              ...
              venue: {
                ok: status.venue && status.venue.ok,
                title: status.venue && status.venue.title,
              },
              settings: {
                webFrontendUrl:
                  status.settings && status.settings.webFrontendUrl,
                webAppUrl: status.settings && status.settings.webAppUrl,
                monthlyCheckWindowStart:
                  status.settings && status.settings.monthlyCheckWindowStart,
                monthlyCheckWindowEnd:
                  status.settings && status.settings.monthlyCheckWindowEnd,
                monthlyReminderStartDay:
                  status.settings && status.settings.monthlyReminderStartDay,
                dailyWorkCheckEnabled:
                  status.settings && status.settings.dailyWorkCheckEnabled,
              },
            };
            break;
          }
          ...
          case "setupOfficialDocumentMonitor": {
            ...
            result = {
              ...
              action,
              sheetNames: [
                OFFICIAL_DOC_QUEUE_SHEET_NAME,
                OFFICIAL_DOC_RUN_LOG_SHEET_NAME,
              ],
            };
            break;
          }
          ...
          case "pendingApprovalStatus": {
            const minAgeHours = e.parameter.minAgeHours
              ? Number(e.parameter.minAgeHours)
              : 0;
          ...
          case "pendingApprovalReminder": {
            result = Object.assign(
              ...
              pendingApprovalReminderJob_({
                dryRun:
                  String(e.parameter.dryRun || "").toLowerCase() !== "false",
                force:
                  String(e.parameter.force || "").toLowerCase() === "true" ||
                  e.parameter.force === "1",
                minAgeHours: e.parameter.minAgeHours
                  ? Number(e.parameter.minAgeHours)
                  : undefined,
                today: e.parameter.date
                  ? parseISODate_(e.parameter.date)
                  : undefined,
              }),
            );
          ...
          case "fetchPdf": {
            ...
            if (!isUnderArchiveRoot_(fid))
              throw new Error("該檔案非系統歸檔範圍");
            const file = DriveApp.getFileById(fid);
            if (file.getMimeType() !== "application/pdf") {
              throw new Error(
                "該檔案非 PDF（mimeType: " + file.getMimeType() + "）",
              );
            }
...
function doPost(e) {
  try {
    ...
    // - 設了 → 嚴格比對
    if (
      typeof handleLineWebhook_ === "function" &&
      raw.indexOf('"events"') >= 0 &&
      raw.indexOf('"destination"') >= 0
    ) {
      const expected = getLineWebhookQueryToken_();
      const provided = (e.parameter && e.parameter.lineWebhookToken) || "";
      if (!expected || expected.length < 32) {
        Logger.log(
          "[LINE webhook] LINE_WEBHOOK_QUERY_TOKEN 未設置，拒絕 webhook",
        );
...
function friendlyError_(err) {
  ...
  const businessErrors = [
    ...
    "adminToken",
    "此 action 需 adminToken",
    "破壞性 action",
    "ADMIN_TOKEN",
    "ALLOW_DESTRUCTIVE_HTTP",
    "webhook_token",

...
function createChecklistDoc_(formType, ctx) {
  ...
  const isDigitalDailyConfirmation =
    isDaily && ctx && ctx.payload && ctx.payload.digitalConfirmation === true;
  ...
  const submittedAtStr = Utilities.formatDate(
    ctx.submittedAt,
    tz_(),
    "yyyy/MM/dd HH:mm",
  );
  ...
  try {
    ...
    if (isClassroomMonthlySafetyPpePdf_(formType, ctx)) {
      appendClassroomMonthlySafetyPpePdf_(
        body,
        ctx,
        rocDateStr,
        submittedAtStr,
    ...
    if (isDaily) {
      ...
      if (schema === "simple") {
        ...
        const hasSections = ctx.payload.items.some((it) =>
          getPdfItemSection_(it),
        );
        let lastSection = "";
        ctx.payload.items.forEach((it) => {
        ...
        // crane_full：固定式起重機原始月檢格式（不另設目視／測試、風險評估或定期檢討欄）
        rows = [["項次", "檢查部分", "檢查內容及方法", "結果"]];
        colWidths = [30, 110, 240, 80]; // 共 460
        ctx.payload.items.forEach((it) => {
          rows.push([
            String(it.order),
            it.name,
    ...
    if (!isDaily && schema === "crane_full") {
      const improvements = ctx.payload.items
        .filter((it) => it.result === "X")
        .map(
          (it) =>
            `${it.order}. ${it.name}：${it.action || it.abnormalDesc || "待改善"}`,
        );
      body.appendParagraph(
        "依檢查結果應採取改善措施：" +
          (improvements.length ? "\n" + improvements.join("\n") : " 無"),
    ...
    if (ctx.payload.signature) {
      ...
      if (!sigBlob)
        throw new Error(
          "dataUrlToBlob_ 回 null，signature 開頭: " +
            ctx.payload.signature.substring(0, 50),
        );
    ...
    if (itemsWithPhotos.length > 0) {
      ...
      itemsWithPhotos.forEach((it) => {
        it.photos.forEach((p, pi) => {
          ...
          // 項次 + 名稱
          const labelP = body.appendParagraph(
            `第 ${it.order} 項：${itemNameWithPdfSection_(it)}`,
          );
          labelP
            .editAsText()
            .setFontSize(12)
          ...
          if (it.abnormalDesc || it.note) {
            const descP = body.appendParagraph(
              "異常說明：" + (it.abnormalDesc || it.note),
            );
          ...
          // 照片
          try {
            const photoBlob = dataUrlToBlob_(
              p,
              `item${it.order}_photo${pi + 1}.jpg`,
            );
            if (photoBlob) {
              body.appendParagraph("");
              ...
              const maxW = 480,
                maxH = 600;
              const w = photoImg.getWidth(),
                h = photoImg.getHeight();
              if (w > maxW || h > maxH) {
                const scale = Math.min(maxW / w, maxH / h);
                photoImg.setWidth(Math.round(w * scale));
                photoImg.setHeight(Math.round(h * scale));
              }
              const cap = body.appendParagraph(
                `照片 ${pi + 1} / ${it.photos.length}`,
              );
...
function appendSupervisorApprovalToDoc_(
  docId,
  supervisorName,
  supervisorSignature,
  approvedAt,
  ...
  const body = doc.getBody();
  const approvedAtStr = Utilities.formatDate(
    approvedAt,
    tz_(),
    "yyyy/MM/dd HH:mm",
  );
...
function isClassroomMonthlySafetyPpePdf_(formType, ctx) {
  ...
  const id = String(ctx.equipment.equipmentId || "")
    .trim()
    .toUpperCase();
  return (
    [
...
function appendClassroomMonthlySafetyPpePdf_(
  body,
  ctx,
  rocDateStr,
  submittedAtStr,
  ...
  const titleP = body.appendParagraph(
  ...
  const resultOptions = (ctx.template && ctx.template.resultOptions) || [
    "ˇ",
    "X",
  ];
  const regularRows = [
  ...
  regularItems.forEach((it) => {
    const parsed = parseMonthlySafetyPpeMethod_(
  ...
  body.appendParagraph("");
  const ruleText =
    ctx.template && ctx.template.rule
  ...
  body.appendParagraph("");
  const submitP = body.appendParagraph(
...
function parseMonthlySafetyPpeMethod_(methodText) {
  let text = String(methodText || "")
    .replace(/\s+/g, " ")
    .trim();
  text = text
    .replace(/^數量\s*\/\s*外觀\s*\/\s*操作\s*[；;:：]?\s*/, "")
    .trim();
  const parts = text
    .split(/[；;]/)
    .map((s) => s.trim())
  ...
  const m = quantityUnit.match(
    /^應備\s*([0-9０-９一二三四五六七八九十]+)\s*(.+)$/,
  );
  return {
    quantity: m ? m[1] : "",
    unit: m ? m[2] : "",
...
function styleClassroomMonthlyGenericTable_(
  table,
  widths,
  resultOptions,
  resultCol,
  ...
  const opts =
    resultOptions && resultOptions.length ? resultOptions : ["ˇ", "X"];
...
function styleClassroomMonthlyResultCell_(cell, goodValue, badValue) {
  ...
  const paintAll = (token, color) => {
    ...
    for (
      ...
      t.setFontSize(idx, end, 14)
        .setBold(idx, end, true)
        .setForegroundColor(idx, end, color);
    }
  };
...
function styleItemsTable_(table, colWidths, isDaily, resultOptions) {
  ...
  const setCenter = (cell) => {
    try {
      cell.getChild(0).asParagraph().setAlignment(HCenter);
    } catch (_) {}
  };

  const opts =
    resultOptions && resultOptions.length ? resultOptions : ["V", "/", "X"];
  const goodValue = opts[0];
  const badValue = opts[opts.length - 1];
  ...
  for (let r = 0; r < numRows; r++) {
    ...
    const numCols = row.getNumCells();
    const isSectionRow =
      !isDaily && r > 0 && row.getCell(0).editAsText().getText() === "";
    for (let c = 0; c < numCols; c++) {
      const cell = row.getCell(c);
      cell
        .setPaddingTop(4)
      ...
      if (r === 0) {
        ...
        if (isDaily && c === 2) {
          ...
          else if (v === goodValue || v === "已確認")
            text.setForegroundColor("#137333").setBold(true);
          else text.setForegroundColor("#666666");
        }
        // 月檢結果欄（固定式起重機與簡式月檢皆在 c=3）
        if (!isDaily && c === 3) {
          const v = text.getText();
          if (v === badValue) text.setForegroundColor("#c5221f").setBold(true);
          else if (v === goodValue)

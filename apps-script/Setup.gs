...
const FIXED_CRANE_MONTHLY_TEMPLATE_ROW_ = [
  ...
  "每月",
  "職業安全衛生管理辦法 第十九、八十條",
  "檢查結果，良好者打「V」，無該項者打「/」，不良者打「X」並應依檢查結果應採取改善措施。",
  "V,/,X",
  "天車完整版",
  "是",
];

const FIXED_CRANE_MONTHLY_ITEMS_ = [
  [
    "F-CRANE-M",
    1,
    "過捲預防裝置",
    "具有自動遮斷動力及制動之機能，作動安全距離符合規定。",
    true,
  ],
  ["F-CRANE-M", 2, "過負荷預防裝置", "當過負荷時具有自動遮斷動力機能。", true],
  [
    "F-CRANE-M",
    3,
    "防止逸走裝置",
    "無損傷、變形，應具有將機具確實固定之機能。(室外)",
    true,
  ],
  ["F-CRANE-M", 4, "阻擋器、緩衝裝置", "無損傷、歪斜、脫落，機能正常。", true],
  [
    "F-CRANE-M",
    5,
    "直行警報裝置",
    "具駕駛室或遙控器者，機具直行時應能發出警報音響。",
    true,
  ],
  [
    "F-CRANE-M",
  ...
  [
    ...
    9,
    "吊鉤",
    "吊鉤應鍛造成形，能自由圓滑轉動，並不得龜裂或明顯之銹蝕等有之缺陷，且未焊補、電鍍等改造。\n吊鉤槽輪組之鍵板、鎖緊銷、止動螺栓、開口銷等無脫落、鬆動或損傷影響安全動作。\n開口標距寬度未超過原標示尺寸5%。與吊具接觸部分磨損量無超過製造廠之規定值者。(無規定值時，其磨損量不得超過原尺寸之5%)(單位:mm)\n吊鉤應設有防止吊掛用鋼索等自該吊鉤脫落之裝置，且作用良好。",
    true,
  ],
  ["F-CRANE-M", 10, "吊具", "無顯著之變形、裂痕。", true],
  [
    "F-CRANE-M",
    11,
    "供電線、配線",
    "絕緣被覆無損傷或老化、無過度張開、扭結或固定夾鬆弛現象。",
    true,
  ],
  [
    "F-CRANE-M",
  ...
  [
    ...
    13,
    "配電盤",
    "檢查配線接頭確實接牢、遮斷器之開關、閘刀開關、電磁接觸器等機能無異常。",
    true,
  ],
  [
    "F-CRANE-M",
...
function getFixedCraneMonthlySourceDefinition_() {
  return {
    templateRow: FIXED_CRANE_MONTHLY_TEMPLATE_ROW_,
    items: FIXED_CRANE_MONTHLY_ITEMS_,
  };
}
...
function setupSecurityProperties() {
  ...
  if (!webhookToken) {
    ...
    report.push(
      "  ⚠ 需到 LINE Developers Console 把 Webhook URL 改成 exec_url?lineWebhookToken=" +
        newToken,
    );
  } else {
  ...
  if (!destructive) {
    report.push(
      "• ALLOW_DESTRUCTIVE_HTTP 未設（HTTP cleanupAll/cleanupDate 將被拒絕，須在編輯器手動執行 cleanupAllSubmissionsAndIncidents_）",
    );
  } else {
...
function initializeDatabase() {
  ...
  setupSheet_(
    ...
    [
      [
        "venueSheetId",
        CONFIG.VENUE_SHEET_ID_FALLBACK,
        "場地使用試算表 ID（每年新表只要改這裡）",
      ],
      [
        "webAppUrl",
        "",
        "Apps Script 部署後的 exec URL（部署完執行 setWebAppUrlFromCurrent 自動填）",
      ],
      [
        "webFrontendUrl",
        "",
        "GitHub Pages 前端網址（提醒信會帶這連結）例：https://<your-github-username>.github.io/auto-checklist",
      ],
    ],
  );
  ensureSystemSettingDefaults_(ss, [
    [
      ...
      "1",
      "月檢應檢期起始日（教室/堆高機/固定式起重機狀態顯示用）",
    ],
    [
      "monthlyCheckWindowEnd",
      "5",
      "月檢應檢期結束日（教室/堆高機/固定式起重機狀態顯示用）",
    ],
    [
    ...
    [
      ...
      "否",
      "是否啟用舊版每日作業檢核；目前改由公文待發文雲端檢核試行，預設停用",
    ],
    [
      "dailyPpeAssignmentEnabled",
      "是",
      "是否啟用每日場地防護具未填提醒；場地有使用且 17:00 後尚未填防護具日檢時，提醒指定窗口人工催填",
    ],
    [
      "dailyPpeAssignmentTriggerHour",
      "17",
      "每日場地防護具未填提醒觸發時段（24 小時制）；預設 17 點",
    ],
    [
      "dailyPpeAssignmentNearMinute",
      "15",
      "每日場地防護具未填提醒約略分鐘；Apps Script 會在此分鐘前後約 15 分鐘內觸發，15 約等於 17:00-17:30",
    ],
    [
      "dailyPpeReminderRecipientNames",
      "卓小媛,張家豪",
      "每日場地防護具未填提醒收件窗口；以逗號、頓號或換行分隔，需在訂閱者清單內且是否訂閱=是",
    ],
    [
      "dailyPpeAssignmentCandidateNames",
      "",
      "舊版每日場地防護具候選名單設定；目前已停用，保留空值供相容舊資料",
    ],
    [
      "dailyPpeStatusLookbackDays",
      "14",
      "舊版 LINE「狀態」每日防護具待確認顯示天數；目前已停用，保留設定供相容舊資料",
    ],
    [
      "dailyPpeResendMinAgeDays",
      "1",
      "舊版主管一鍵補發每日防護具待確認門檻；目前已停用，保留設定供相容舊資料",
    ],
    [
      "lineRichMenuImageUrl",
      "https://isha-taichung.github.io/auto-checklist/assets/line-rich-menu-main.png",
      "LINE 圖文選單圖片網址（2500x1686 PNG）",
    ],
    [
      "venueUsageRequiredKeywords",
      "堆高機=堆;高空工作車=高",
      "共用場地分頁的使用判定必要關鍵字；格式：設備類別=關鍵字1,關鍵字2；例：高空工作車內容需含「高」才算高空工作車有使用",
    ],
  ]);
  ...
  // - monthlySchema：crane_full = 固定式起重機原始月檢格式（部份/檢查內容及方法/結果）
  ...
  setupSheet_(
    ...
    [
      [
        ...
        "每日",
        "職業安全衛生管理辦法 §52",
        "良好「V」/ 無此項「/」/ 不良「X」（不良需於記事欄註明）",
        "V,/,X",
        "",
        "是",
      ],
      FIXED_CRANE_MONTHLY_TEMPLATE_ROW_,
      [
        "F-FORK-D",
        "堆高機",
        "堆高機每日使用前檢點表",
      ...
      [
        ...
        "車載式高空工作車每日作業前檢點表",
        "每日",
        "依附件「車載式高空工作車每日作業前檢點表」",
        "檢查結果正常打「V」，異常打「X」；異常項目需提出改善措施",
        "V,X",
        "",
        "是",
      ],
      [
        ...
        "自走式高空工作車每日作業前檢查表",
        "每日",
        "依附件「高空作業車作業前檢查表（自主檢查表）」",
        "每日檢查合格後打「ˇ」，異常打「X」；異常應停止使用並通知相關人員處理",
        "ˇ,X",
        "",
        "是",
      ],
  ...
  setupSheet_(
    ...
    [
      ...
      [
        "F-AWP-D",
        1,
        "昇空臂、昇空桶有無損壞、裂痕、變形，昇空桶有無鑽孔或底部有無破洞",
        "目視檢查",
        true,
      ],
      [
        "F-AWP-D",
        2,
        "鋼構與昇空臂絕緣接合處外觀有無龜裂、變形、接合鬆動，昇空臂搖晃是否過大",
        "目視檢查",
        true,
      ],
      ...
      [
        "F-AWP-D",
        4,
        "昇空臂基座介面有無鬆弛龜裂變形、結構配件有無鬆動或遺失、螺栓螺帽有無鬆弛或脫落及是否旋緊密合、各部組件功能是否完妥、是否搖晃過大或鬆動異常聲響",
        "目視檢查",
        true,
      ],
      [
        "F-AWP-D",
        5,
        "各高壓油管及各接頭有否洩漏及定位（含基座、旋轉軸承及齒輪箱、上下臂之油壓缸、昇空桶油管）",
        "目視檢查",
        true,
      ],
      [
        "F-AWP-D",
        6,
        "各組件焊接部位有無變形（含基座、旋轉、撐腳、上臂肘、上臂頂、下臂、昇空桶）",
        "目視檢查",
        true,
      ],
      [
      ...
      [
        "F-AWP-D",
        14,
        "煞車器、離合器、開關控制器、動力傳動器（PTO）、油壓泵運轉是否正常",
        "試車動作測試",
        true,
      ],
      ...
      [
        "F-AWP-SD",
        17,
        "【工作車輛】前進後退、舉升及制動裝置、傾斜度、水平測試是否正常",
        "測試",
        true,
      ],
      [
  ...
  Logger.log(
    "資料庫初始化完成。請接著到 Sheets 確認 → 回 Apps Script 執行 installDailyReminderTrigger",
  );
}
...
function applyProjectResourceNames() {
  ...
  try {
    DriveApp.getFileById(ScriptApp.getScriptId()).setName(
      "ISHA 檢查與通報 API",
    );
  ...
  result.machineIncidentSheetName = machineIncidentSheet
    ? machineIncidentSheet.getName()
    : "";
  ...
  if (typeof ensureDailyIncidentSettings_ === "function") {
    ensureDailyIncidentSettings_(ss);
    result.dailyIncidentArchiveFolderName = getSetting_(
      "dailyIncidentArchiveFolderName",
      "",
    );
  }
...
function setupSheet_(ss, sheetName, headers, initialRows) {
  ...
  const existingHeaders = existingRow
    .map((v) => String(v || "").trim())
    .filter(Boolean);
...
function ensureSystemSettingDefaults_(ss, rows) {
  ...
  if (toAppend.length > 0) {
    sheet
      .getRange(sheet.getLastRow() + 1, 1, toAppend.length, 3)
      .setValues(toAppend);
  }
}
...
function getSubscriberSheetHeaderNotes_() {
  return {
    ...
    是否訂閱:
      "主動推播總開關。是＝允許系統主動推播；否＝仍可使用指令查詢，但不收主動通知。這是試算表控管，不是 LINE@ 後台的好友狀態。",
    是否為主管: "主管身分。是＝可收到主管簽核、主管審閱等主管通知。",
    是否為同仁:
      "同仁身分。是＝可作為日常事件填報/承辦人、公文待發文承辦人等對象。",
    機具設備異常:
      "細分通知開關。只有填「是」才會收到機具設備檢查異常通知；空白或否＝不收。",
    機具設備日檢點未填:
      "細分通知開關。只有填「是」才會收到機具設備日檢未填提醒；空白或否＝不收。",
    機具設備月檢點未填:
      "細分通知開關。只有填「是」才會收到機具設備月檢未填提醒；空白或否＝不收。",
    三間教室月檢:
      "細分通知開關。只有填「是」才會收到三間教室月檢未填提醒；空白或否＝不收。月檢送審簽核通知只看「是否為主管」。",
  ...
  headers.forEach((header, i) => {
    ...
    if (notes[header]) {
      try {
        ...
        Logger.log("訂閱者清單欄位顯示失敗：" + header + " / " + e);
      }
    }
  });
}
...
function getSupervisorStatus_() {
  ...
  const targetIds = new Set(
    (props.getProperty("LINE_TARGET_USER_IDS") || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
  );
  ...
  const notificationColumns =
    typeof LINE_NOTIFICATION_COLUMNS !== "undefined"
      ? Object.keys(LINE_NOTIFICATION_COLUMNS).map(
          (k) => LINE_NOTIFICATION_COLUMNS[k],
        )
      : [];
  ...
  const subscribers = data.map((row, i) => {
    const id = String(row[idCol] || "").trim();
    const subscribed =
      subscribeCol < 0 ? true : isActiveValue_(row[subscribeCol]);
    const isSupervisor =
      supervisorCol >= 0 ? isActiveValue_(row[supervisorCol]) : false;
    ...
    notificationColumns.forEach((colName) => {
      ...
      notificationFlags[colName] =
        col >= 0 ? isLineNotificationEnabled_(row[col]) : false;
    });
    return {
      rowNo: i + 2,
  ...
  return {
    ...
    subscribedNotInLegacyTargetCount: subscribedRows.filter(
      (s) => !s.inGeneralTarget,
    ).length,
    activeNotInGeneralTargetCount: subscribedRows.filter(
      (s) => !s.inGeneralTarget,
    ).length,
    subscribers,
    supervisors,
...
function cleanupAllSubmissionsAndIncidents_(opts) {
  ...
  if (recSheet) {
    ...
    if (dataRows > 0) {
      ...
      if (idxPdf >= 0) {
        const urls = recSheet
          .getRange(2, idxPdf + 1, dataRows, 1)
          .getValues()
          .flat();
        urls.forEach((u) => {
      ...
      if (idxDraft >= 0) {
        const ids = recSheet
          .getRange(2, idxDraft + 1, dataRows, 1)
          .getValues()
          .flat();
        ids.forEach((id) => {
          const s = String(id || "").trim();
      ...
      if (!dryRun) {
        pdfsToTrash.forEach((fid) => {
          try {
            ...
            pdfErrors++;
            Logger.log(`[cleanupAll] PDF ${fid} 失敗: ${e}`);
          }
        });
        // 全部刪除（保留 header）
    ...
    report.push(
      `${dryRun ? "[DRY-RUN]" : "✓"} 填報紀錄：${dryRun ? "會刪" : "刪除"} ${dataRows} 列、${dryRun ? "會 trash" : "trashed"} ${dryRun ? pdfsToTrash.length : trashedPdfs} PDF${pdfErrors ? `（PDF 失敗 ${pdfErrors}）` : ""}`,
    );
  }
  ...
  if (incSheet) {
    ...
    report.push(
      `${dryRun ? "[DRY-RUN]" : "✓"} 機具設備異常事件：${dryRun ? "會刪" : "刪除"} ${dataRows} 列`,
    );
  }
...
function cleanupTestDataForDate_(dateStr, opts) {
  ...
  if (recSheet && recSheet.getLastRow() >= 2) {
    ...
    report.push(
      `${dryRun ? "[DRY-RUN]" : "✓"} 填報紀錄：${dryRun ? "會刪" : "刪除"} ${recRowsToDelete.length} 列、${dryRun ? "會 trash" : "trashed"} ${dryRun ? recPdfsToTrash.length : trashedPdfs} PDF${pdfErrors ? `（失敗 ${pdfErrors}）` : ""}`,
    );
  }
  ...
  if (incSheet && incSheet.getLastRow() >= 2) {
    ...
    report.push(
      `${dryRun ? "[DRY-RUN]" : "✓"} 機具設備異常事件：${dryRun ? "會刪" : "刪除"} ${incRowsToDelete.length} 列`,
    );
  }
...
function markIncidentsCompletedForEquipment_(equipmentId, formType) {
  ...
  const targetType =
    formType === "daily" ? "每日" : formType === "monthly" ? "每月" : null;
...
function applyColumnWidthsAndWrap_() {
  ...
  Object.keys(profiles).forEach((sheetName) => {
    ...
    wrapCols.forEach((col) => {
      ...
      if (i > 0)
        sheet
          .getRange(1, i, Math.max(sheet.getMaxRows(), 100), 1)
          .setWrap(true);
    });
...
function applyChineseSettingsAndDropdowns() {
  ...
  Object.keys(HEADER_RENAMES).forEach((sheetName) => {
    ...
    if (renamed > 0)
      report.push(`✓ 「${sheetName}」重新命名 ${renamed} 個英文欄位 → 中文`);
  });
  ...
  const SHEET_PROFILES = {
    ...
    每日作業檢核: [
      {
        col: "15天後課程是否報備",
        options: DAILY_WORK_CHECK_OPTIONS || ["是", "否", "不適用"],
        strict: true,
      },
      {
        col: "1天後異動是否完成",
    ...
    公文待發文佇列: [
      {
        col: "通知狀態",
        options: ["待通知", "已通知", "查無LINE", "通知失敗", "略過"],
        strict: true,
      },
    ],
    工作日例外: [
    ...
    日常異常事件通報: [
      {
        col: "填報事項",
        options: DAILY_INCIDENT_SUBJECTS || [
          "環境設施",
          "場地使用",
          "安全衛生",
          "人員反映",
      ...
      {
        col: "處理狀況",
        options: DAILY_INCIDENT_PROCESS_STATUSES || [
          "待處理",
          "處理中",
          "處理完成",
        ],
      ...
      {
        col: "審核狀態",
        options: DAILY_INCIDENT_REVIEW_STATUSES || [
          "未送審",
          "待主管審核",
          "已結案",
          "退回補正",
  ...
  Object.keys(SHEET_PROFILES).forEach((sheetName) => {
    const sheet = ss.getSheetByName(sheetName);
    if (!sheet) {
      report.push(`⚠ 工作表「${sheetName}」不存在，略過`);
      return;
    }
    ...
    SHEET_PROFILES[sheetName].forEach(({ col, options, migrate, strict }) => {
      ...
      if (colIdx < 1) {
        report.push(`⚠ 「${sheetName}」找不到欄位「${col}」，略過`);
        return;
      }
      if (!options || options.length === 0) {
        report.push(`⚠ 「${sheetName}.${col}」沒有可用選項，略過`);
        return;
      }
      ...
      const validationRange = sheet.getRange(
        2,
        colIdx,
        Math.max(sheet.getMaxRows() - 1, 100),
        1,
      );
      const rule = SpreadsheetApp.newDataValidation()
      ...
      report.push(
        `✓ 「${sheetName}.${col}」加下拉（${options.length} 個選項）${migratedCount > 0 ? ` + 遷移 ${migratedCount} 列` : ""}`,
      );
    });
  });
  ...
  if (tplSheet) {
    ...
    for (let i = 1; i < tData.length; i++) {
      ...
      // 先修正已知舊格式
      const forced = FORCED_VALUE[id];
      if (forced) {
        for (const col of Object.keys(forced)) {
          const ci = colMap[col];
          if (ci < 0) continue;
          const cur = String(tData[i][ci] || "").trim();
          if (cur !== forced[col]) {
            tplSheet.getRange(i + 1, ci + 1).setValue(forced[col]);
            corrected++;
          }
        }
      }
    ...
    if (corrected > 0)
      report.push(`✓ 修正「檢查表模板」舊格式 ${corrected} 格`);
    if (backfilled > 0)
      report.push(`✓ 補填「檢查表模板」空白欄位 ${backfilled} 格`);
  }
 ...
 */
function migrateFixedCraneMonthlyChecklistToSource_() {
  const ss = SpreadsheetApp.openById(CONFIG.DB_SHEET_ID);
  const tplSheet = ss.getSheetByName("檢查表模板");
  const itemSheet = ss.getSheetByName("檢查項目");
  if (!tplSheet || !itemSheet)
    throw new Error("找不到固定式起重機月檢所需資料表");

  const tplHeaders = tplSheet
    .getRange(1, 1, 1, tplSheet.getLastColumn())
    .getValues()[0];
  ...
  templateColumns.forEach((name) => {
    if (tplIdx(name) < 0) throw new Error("檢查表模板缺欄位：" + name);
  });

  const tplData = tplSheet.getDataRange().getValues();
  let templateRow = -1;
  for (let r = 1; r < tplData.length; r++) {
    if (String(tplData[r][tplIdx("表單ID")] || "").trim() === "F-CRANE-M") {
      templateRow = r + 1;
      break;
    }
  }
...
function addForkliftTemplatesAndItems() {
  ...
  if (tplLastRow > 1) {
    tplSheet
      .getRange(2, tplIdx("表單ID") + 1, tplLastRow - 1, 1)
      .getValues()
  ...
  if (itemLastRow > 1) {
    const data = itemSheet
      .getRange(2, 1, itemLastRow - 1, itemHeaders.length)
      .getValues();
    data.forEach((r) => {
...
function addForkliftEquipments() {
  ...
  if (lastRow > 1) {
    sheet
      .getRange(2, idx("設備代號") + 1, lastRow - 1, 1)
      .getValues()
  ...
  Logger.log(
    "新增 " +
      newRows.length +
      " 台堆高機：" +
      letters.slice(0, newRows.length).join(", "),
  );
...
function addAerialWorkPlatformTemplatesAndEquipment() {
  ...
  if (tplIdCol >= 0 && tplSheet.getLastRow() > 1) {
    tplSheet
      .getRange(2, 1, tplSheet.getLastRow() - 1, tplHeaders.length)
      .getValues()
      .forEach((r, i) =>
        existingTplRows.set(String(r[tplIdCol] || "").trim(), i + 2),
      );
  }
  const tplAppend = [];
  ...
  if (oldMonthlyTplRow) {
    ...
    if (activeCol >= 0)
      tplSheet.getRange(oldMonthlyTplRow, activeCol + 1).setValue("否");
    if (schemaCol >= 0)
      tplSheet.getRange(oldMonthlyTplRow, schemaCol + 1).setValue("");
    result.templatesDisabled += 1;
  }
  if (tplAppend.length) {
    const range = tplSheet.getRange(
      tplSheet.getLastRow() + 1,
      1,
      tplAppend.length,
      tplHeaders.length,
    );
  ...
  if (formIdCol >= 0 && orderCol >= 0 && itemSheet.getLastRow() > 1) {
    itemSheet
      .getRange(2, 1, itemSheet.getLastRow() - 1, itemHeaders.length)
      .getValues()
      .forEach((r, i) =>
        existingItems.set(
          String(r[formIdCol] || "").trim() +
            "|" +
            String(r[orderCol] || "").trim(),
          i + 2,
        ),
      );
  }
  ...
  if (formIdCol >= 0 && activeItemCol >= 0 && itemSheet.getLastRow() > 1) {
    itemSheet
      ...
      .forEach((r, i) => {
        if (
          String(r[formIdCol] || "").trim() === "F-AWP-M" &&
          isActiveValue_(r[activeItemCol])
        ) {
  ...
  if (itemAppend.length) {
    const range = itemSheet.getRange(
      itemSheet.getLastRow() + 1,
      1,
      itemAppend.length,
      itemHeaders.length,
  ...
  const eqSheet = ss.getSheetByName("設備清單");
  const eqHeaders = eqSheet
    .getRange(1, 1, 1, eqSheet.getLastColumn())
    .getValues()[0]
    .map((h) => String(h || "").trim());
  ...
  if (eqIdCol >= 0 && eqSheet.getLastRow() > 1) {
    eqSheet
      .getRange(2, 1, eqSheet.getLastRow() - 1, eqHeaders.length)
      .getValues()
      .forEach((r, i) =>
        existingEqRows.set(String(r[eqIdCol] || "").trim(), i + 2),
      );
  }
  const equipmentRows = [
    {
  ...
  equipmentRows.forEach((eqp) => {
    ...
    const setE = (name, value) => {
      const i = eqHeaders.indexOf(name);
      if (i >= 0) row[i] = value;
    };
    setE("設備代號", eqp.id);
    setE("設備名稱", eqp.name);
    ...
    const range = eqSheet.getRange(
      eqSheet.getLastRow() + 1,
      1,
      1,
      eqHeaders.length,
    );
  ...
  result.settingsUpdated = ensureVenueUsageKeywordSetting_(
    ss,
    "高空工作車",
    "高",
  )
  ...
  SpreadsheetApp.flush();
  Logger.log(
    "addAerialWorkPlatformTemplatesAndEquipment 完成：" +
...
function ensureVenueUsageKeywordSetting_(ss, category, keyword) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (noteCol >= 0)
      sheet
        .getRange(i + 1, noteCol + 1)
        .setValue(
          "共用場地分頁的使用判定必要關鍵字；例：堆高機需含「堆」、高空工作車需含「高」才算使用",
        );
    return true;
  }
...
function addPpeTemplatesAndEquipments() {
  ...
  if (tplLastRow > 1 && tplIdCol >= 0) {
    tplSheet
      .getRange(2, tplIdCol + 1, tplLastRow - 1, 1)
      .getValues()
      .forEach((r, i) => {
  ...
  if (!existingTplIds.has("F-PPE-D")) {
    ...
    const setT = (name, v) => {
      const i = tplHeaders.indexOf(name);
      if (i >= 0) tplRow[i] = v;
    };
    setT("表單ID", "F-PPE-D");
    setT("設備類別", "防護具檢點");
    ...
    setT("啟用", "是");
    tplSheet
      .getRange(tplSheet.getLastRow() + 1, 1, 1, tplHeaders.length)
      .setValues([tplRow]);
  ...
  ppeItems.forEach((it) => {
    ...
    const row = new Array(itemHeaders.length).fill("");
    const setI = (name, v) => {
      const i = itemHeaders.indexOf(name);
      if (i >= 0) row[i] = v;
    };
    setI("表單ID", "F-PPE-D");
    setI("項目順序", it.order);
  ...
  if (eqLastRow > 1) {
    ...
    eqSheet
      .getRange(2, idCol + 1, eqLastRow - 1, 1)
      .getValues()
  ...
  if (newEqRows.length > 0) {
    eqSheet
      .getRange(eqSheet.getLastRow() + 1, 1, newEqRows.length, eqHeaders.length)
      .setValues(newEqRows);
...
function getSystemStatus_() {
  ...
  // 3. 各表筆數
  const counts = {};
  ["設備清單", "檢查表模板", "檢查項目", "填報紀錄", "節假日關鍵字"].forEach(
    (name) => {
  ...
  return {
    ...
    dbSheetUrl:
      "https://docs.google.com/spreadsheets/d/" + CONFIG.DB_SHEET_ID + "/edit",
    settings,
    triggers,
    counts,

...
function handleSubmission_(payload) {
  ...
  if (equipmentForTpl) {
    tplForValidation = getTemplateForCategoryCycle_(
      equipmentForTpl.category,
      payload.formType,
      equipmentForTpl,
    );
    if (!tplForValidation) {
      throw new Error(
        "找不到模板：" + equipmentForTpl.category + " - " + payload.formType,
      );
    }
  }
  ...
  const fallbackResult = {
    daily: ["V", "/", "X"],
    // 若模板設定尚未同步，固定式起重機月檢仍須接受原表 V／／／X。
    monthly:
      tplForValidation && tplForValidation.monthlySchema === "crane_full"
        ? ["V", "/", "X"]
        : ["normal", "abnormal"],
  };
  const allowedResults =
    ...
    tplForValidation.resultOptions.length
      ? tplForValidation.resultOptions
  ...
  payload.items = payload.items.map((it, idx) => {
    ...
    if (Array.isArray(it.photos)) {
      if (it.photos.length > CONFIG.MAX_PHOTOS_PER_ITEM) {
        throw new Error(
          `第 ${idx + 1} 項照片超過 ${CONFIG.MAX_PHOTOS_PER_ITEM} 張上限`,
        );
      }
      photos = it.photos
    ...
    return {
      ...
      methods,
      method: sanitizeText_(it.method, 1200), // 固定式起重機月檢需保留完整檢查內容及方法
  ...
  try {
    ...
    const tplMeta = getTemplateForCategoryCycle_(
      equipment.category,
      payload.formType,
      equipment,
    );
    if (!tplMeta)
      throw new Error(
        "找不到模板：" + equipment.category + " - " + payload.formType,
      );

    const needsApproval = requiresSupervisorApproval_(
      payload.formType,
      equipment,
    );
    let approvalToken = "";
    ...
    if (needsApproval) {
      ...
      if (!approvalUrl)
        throw new Error("系統設定 webAppUrl 未填，無法建立主管簽核連結");
      ...
      pdfBlob.setName(
        buildPdfFilename_(payload.formType, checkDate, equipment),
      );
    ...
    try {
      ...
      Logger.log(
        "writeIncidents_ 失敗（主紀錄已寫，異常追蹤可從 JSON 重建）: " +
          incidentErr +
          "\n" +
          (incidentErr.stack || ""),
      );
...
function writeRecord_({
  ...
  if (Array.isArray(payloadForLog.items)) {
    payloadForLog.items = payloadForLog.items.map((it) =>
      Object.assign({}, it, {
        photos: Array.isArray(it.photos)
          ? `[${it.photos.length} 張照片，已嵌入 PDF]`
          : undefined,
      }),
    );
  }
...
function requiresSupervisorApproval_(formType, equipment) {
  ...
  return (
    ["堆高機", "固定式起重機"].indexOf(
      String(equipment.category || "").trim(),
    ) >= 0
  );
}
...
function notifySupervisorApprovalRequest_(record) {
  if (typeof sendApprovalRequest_ !== "function")
    return { ok: false, reason: "missing_sendApprovalRequest" };
...
function resendSupervisorApprovalRequest_(opts) {
  ...
  const data = sheet
    ...
    .getValues();
  const parsedRows = data.map((row, i) =>
    approvalRecordFromRow_(sheet, headers, row, i + 2),
  );
  ...
  for (let i = parsedRows.length - 1; i >= 0; i--) {
    ...
    if (
      targetEquipmentName &&
      rec.equipmentName.indexOf(targetEquipmentName) < 0
    )
      continue;
  ...
  if (matches.length > 1 && !opts.latest && !targetRecordId) {
    return {
      ...
      matches: matches
        .slice(0, 10)
        .map((m) => approvalResendSafeRecord_(m.rec)),
    };
  }
  ...
  const supervisorIds =
    typeof getSupervisorUserIds_ === "function" ? getSupervisorUserIds_() : [];
  if (dryRun) {
    return {
      ok: true,
...
function listPendingApprovalRecords_(opts) {
  ...
  for (let i = 0; i < parsedRows.length; i++) {
    ...
    const submittedRaw =
      submittedIdx >= 0 ? row[submittedIdx] : rec.submittedAt;
    const submittedAt = approvalParseDateTime_(submittedRaw || rec.submittedAt);
    const ageHours = submittedAt
      ? Math.max(
          0,
          Math.floor((now.getTime() - submittedAt.getTime()) / 3600000),
        )
      : null;
    if (minAgeHours > 0 && (ageHours == null || ageHours < minAgeHours))
      continue;

    let checkDateLabel = rec.checkDate || "";
    try {
      checkDateLabel = formatROCDate_(parseISODate_(rec.checkDate));
    } catch (_) {}
    records.push({
      recordId: rec.recordId,
  ...
  records.sort((a, b) => {
    ...
    return String(a.equipmentName || "").localeCompare(
      String(b.equipmentName || ""),
      "zh-Hant",
    );
  });
  return records;
...
function diagnoseApprovalBatch_(opts) {
  ...
  const anchor = targetRecordId
    ...
    : records.find((rec) => {
        ...
        if (targetEquipmentId && rec.equipmentId !== targetEquipmentId)
          return false;
        if (
          targetEquipmentName &&
          rec.equipmentName.indexOf(targetEquipmentName) < 0
  ...
  const key = anchor
    ? approvalBatchKey_(anchor)
    : [targetDate, "monthly", targetEquipmentId || targetEquipmentName].join(
        "|",
      );
  ...
  return {
    ...
    records: batchRecords.map((rec) =>
      approvalDiagnosticSafeRecord_(rec, archivedMap),
    ),
  };
}
...
function getPendingApprovalRecordsForLineUser_(userId) {
  const profile =
    typeof getLineSubscriberProfileByUserId_ === "function"
...
function pendingApprovalReminderJob_(opts) {
  ...
  const minAgeHours = Math.max(
    ...
    Number(
      opts.minAgeHours ||
        getSetting_("pendingApprovalReminderMinAgeHours", "24") ||
        24,
    ),
  );
  ...
  if (!records.length)
    return rememberPendingApprovalReminderRun_(resultBase, { dryRun, force });

  const props = PropertiesService.getScriptProperties();
  const sentKey = `PENDING_APPROVAL_REMINDER_SENT_${todayIso}`;
  if (!force && props.getProperty(sentKey) === "YES") {
    return rememberPendingApprovalReminderRun_(
  ...
  if (dryRun) {
    const supervisorIds =
      typeof getSupervisorUserIds_ === "function"
        ? getSupervisorUserIds_()
        : [];
    return rememberPendingApprovalReminderRun_(
      Object.assign({}, resultBase, {
        action: "wouldPush",
  ...
  const pushed =
    typeof sendPendingApprovalReminder_ === "function"
  ...
  return rememberPendingApprovalReminderRun_(
    Object.assign({}, resultBase, pushed || {}, {
...
function rememberPendingApprovalReminderRun_(result, opts) {
  opts = opts || {};
  const key = opts.dryRun
    ? "PENDING_APPROVAL_REMINDER_LAST_DRY_RUN"
    : "PENDING_APPROVAL_REMINDER_LAST_RUN";
  const payload = {
    ...
    pushedCount: result
      ? Number(result.pushedCount || result.successCount || 0)
      : 0,
...
function getPendingApprovalReminderRunStatus_() {
  const props = PropertiesService.getScriptProperties();
  return {
    lastRun: parsePendingApprovalReminderRun_(
      props.getProperty("PENDING_APPROVAL_REMINDER_LAST_RUN"),
    ),
    lastDryRun: parsePendingApprovalReminderRun_(
      props.getProperty("PENDING_APPROVAL_REMINDER_LAST_DRY_RUN"),
    ),
  };
}
...
function handleApprovalSubmission_(payload) {
  ...
  try {
    ...
    appendSupervisorApprovalToDoc_(
      rec.draftDocId,
      supervisorName,
      supervisorSignature,
      approvedAt,
...
function getApprovalRecord_(recordId, token) {
  ...
  const tokenIdx = headers.indexOf("主管簽核Token");
  if (idIdx < 0 || tokenIdx < 0)
    throw new Error("填報紀錄缺簽核欄位，請先執行 initializeDatabase");
  const data = sheet
    .getRange(2, 1, sheet.getLastRow() - 1, headers.length)
    .getValues();
...
function getApprovalSummary_(recordId, token) {
  ...
  const archivedSibling = approvalStatusIsPending_(rec.status)
    ...
    : null;
  const displayStatus = archivedSibling
    ? "已簽核歸檔"
    : rec.status || "待主管簽核";
...
function approvalCheckItems_(rec) {
  ...
  return payload.items.map((it) => ({
    ...
    section: sanitizeText_(it.section, 80),
    name: sanitizeText_(it.name, 200),
    methods: Array.isArray(it.methods)
      ? it.methods.map((m) => sanitizeText_(m, 20)).filter(Boolean)
      : [],
...
function notifyLineIncidents_(
  ...
  payload.items.forEach((it) => {
    ...
    const tracking = machineIncidentSubmissionTracking_(
      it,
      lockedByOrder[it.order],
    );
    incidents.push({
      order: it.order,
...
function writeIncidents_({
  ...
  const allowedResults =
    tplMeta &&
    Array.isArray(tplMeta.resultOptions) &&
    tplMeta.resultOptions.length
...
function machineIncidentSubmissionTracking_(item, locked) {
  const currentDescription =
    abnormalDescriptionWithCheckResults_(item) || "(無說明)";
  if (!locked) {
    return {
  ...
  const normalizedCurrent =
    machineIncidentNormalizeTrackingDescription_(currentDescription);
...
function getTemplateForCategoryCycle_(category, formType, equipment) {
  ...
  ["表單ID", "啟用", "設備類別", "週期"].forEach((col) => {
    if (idx(col) < 0)
      throw new Error(
        "檢查表模板缺必要欄位：" + col + "（請執行 initializeDatabase 補欄）",
      );
  });
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (overrideId) {
      if (
        String(data[i][idx("表單ID")] || "").trim() !== overrideId ||
...
function buildPdfFilename_(formType, checkDate, equipment) {
  ...
  const safeName = (equipment.equipmentName || equipment.equipmentId).replace(
    /[\\/:*?"<>|\x00-\x1f]/g,
    "_",
  );

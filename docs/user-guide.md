...
「待發文」按鈕讀取的是雲端檢核在 16:30 / 17:00 寫入的最新快照，不是點擊後即時登入公文系統。若畫面顯示「今日尚未有檢核紀錄」，代表當日檢核批次尚未跑完或尚未寫入快照。
...
請直接在 ISHA 通知小幫手的一對一聊天中操作 LINE 指令。

若待發文查不到本人資料，請確認 LINE 已在「訂閱者清單」綁定姓名，並且已標記為同仁。
...
| 退回補正 | 系統重產退回補正 PDF，並推送退回補正 LINE 圖卡給訂閱者清單 |
